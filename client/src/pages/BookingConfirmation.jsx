    ...
    <div className="p-5 space-y-4">
      ...
      <div className="border-t border-gray-100 pt-4 space-y-2">
        <div className="flex justify-between text-sm">
          <span className="text-gray-500">₱{(booking.pricePerNight || total / nights).toLocaleString()} × {nights} nights</span>
...
export default function BookingConfirmation() {
  ...
  const currentStepIdx = step === STEPS.INFO ? 0 : step === STEPS.PAYMENT_TYPE ? 1 : 2;
  ...
  const handlePaymentTypeSelect  = (type) => { setPaymentType(type); setStep(STEPS.CONFIRM); };
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      await bookingsAPI.create({
        ...
        arrivalTime:    booking.arrivalTime || '14:00', // ← ADD THIS
        ...
        paymentType,
        status:         'pending',
        userId:         user?.id || null, // ← send user_id if logged in
      });
      ...
      if (data.checkoutUrl) {
        ...
        alert(`Dev mode: Booking saved for ${form.fullName}!`);
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 pt-20 pb-16">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex items-center gap-4 py-8">
          <button onClick={() => step === STEPS.INFO ? navigate(-1) : setStep(prev =>
          ...
          <div className="lg:col-span-3">
            ...
            <AnimatePresence mode="wait">
                ...
                <motion.div key="info" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" placeholder="e.g. Juan dela Cruz" value={form.fullName} onChange={setField('fullName')} className={inputClass(errors.fullName)} />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="email" placeholder="you@email.com" value={form.email} onChange={setField('email')} className={inputClass(errors.email)} />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="tel" placeholder="+63 9XX XXX XXXX" value={form.phone} onChange={setField('phone')} className={inputClass(errors.phone)} />
                    </div>
                  ...
                  <div>
                    ...
                    <textarea rows={3} placeholder="Any special requests or notes for the resort..."
                  ...
                  <button onClick={handleInfoNext} className="w-full bg-ocean-600 hover:bg-ocean-700 text-white font-semibold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2">
              ...
              {step === STEPS.PAYMENT_TYPE && (
                <motion.div key="payment_type" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                  ...
                  <div className="flex items-start gap-3 bg-gray-50 border border-gray-200 rounded-xl p-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">GCash · Maya · Credit/Debit Card · GrabPay — powered by PayMongo.</p>
                    </div>
                  </div>
                  <button onClick={() => setStep(STEPS.INFO)} className="w-full text-sm text-gray-400 hover:text-gray-600 py-2 transition-colors">
                  ...
                  <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                    ...
                    <div className="bg-gray-50 rounded-2xl p-4 mb-4 space-y-2.5 text-sm">
                      ...
                      {[
                        ...
                        ['Arrival Time', formatTime(booking.arrivalTime)],
                    ...
                    <div className={`rounded-2xl p-4 mb-5 border ${paymentType === 'full' ? 'bg-ocean-50 border-ocean-200' : 'bg-emerald-50 border-emerald-200'}`}>
                      <div className="flex items-center gap-2 mb-3">
                        {paymentType === 'full' ? <CreditCard className="w-4 h-4 text-ocean-600" /> : <Percent className="w-4 h-4 text-emerald-600" />}
                      ...
                      <div className="space-y-1.5 text-sm">
                        ...
                        <div className={`flex justify-between pt-2 border-t font-bold text-base ${paymentType === 'full' ? 'border-ocean-200' : 'border-emerald-200'}`}>
                          <span>Pay now</span>
                          <span className={paymentType === 'full' ? 'text-ocean-600' : 'text-emerald-600'}>₱{amountToPay.toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-xl p-3 mb-5">
                      ...
                      <div className="text-xs text-blue-700">
                        ...
                        <p>You'll be redirected to PayMongo's secure checkout.</p>
                      </div>
                    </div>
                    {submitError && (
                    ...
                    <button onClick={handleProceedToPayment} disabled={submitting}
                      className={`w-full text-white font-semibold py-4 rounded-xl transition-colors flex items-center justify-center gap-2 shadow-lg ${
                        paymentType === 'full' ? 'bg-ocean-600 hover:bg-ocean-700 disabled:bg-gray-300' : 'bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-300'
                    ...
                    <p className="text-center text-xs text-gray-400 mt-3">🔒 Your payment is secured and encrypted by PayMongo</p>
                  </div>
                  <button onClick={() => setStep(STEPS.PAYMENT_TYPE)} className="w-full text-sm text-gray-400 hover:text-gray-600 py-2 transition-colors">
                    ← Change payment type
                  </button>
                </motion.div>
              )}
